"""Regression tests for the final experiment and statistics pipeline."""
...
from scripts.run_shap import group
...
def test_seed_definitions_and_optuna_never_receive_test_arrays():
 cfg=json.loads((ROOT/'results/final_experiments/manifest/run_manifest.json').read_text());assert cfg['seeds']==[0,1,2,3,4];src=inspect.getsource(optuna_tuner.tune_xgboost);assert 'X_test' not in src and 'y_test' not in src;assert 'X_val' in src and 'y_val' in src
...
def test_shap_membership_and_b_aggregation():
 ...
 d=pd.read_csv(ROOT/'results/final_experiments/shap/seed_level_group_attribution.csv');f=d[d.representation_id=='A+B+C'];wide=f.pivot(index='seed',columns='group',values='attribution_proportion');assert np.allclose(wide.B,wide.B1+wide.B2+wide.B3)

def test_prediction_alignment_and_primary_completeness():
 d=pd.read_csv(ROOT/'results/final_experiments/primary_ablation/seed_level_results.csv');assert len(d)==70
...
def test_estimator_completeness_and_resume_validation():
 d=pd.read_csv(ROOT/'results/final_experiments/estimator_matrix/seed_level_results.csv');assert len(d)==175;assert (d.status=='completed').all();p=ROOT/'results/final_experiments/raw_runs/primary_ablation/random/seed_0/chem_A_xgb/metrics.json';assert valid(p)
