...
Uses only chemical descriptors (no sequence information).
"""
...
from baselines.featurizers.descriptors import DescriptorOnlyFeaturizer
from data.pem_schema import PEMSample


class DescriptorOnlyBaseline(BaselineModel):
    """
    Descriptor-only baseline.

    Uses only chemical descriptors from modifications (no sequence).
    """

    def __init__(self, config: BaselineConfig):
        """
        Initialize descriptor-only baseline.
        ...
        self.featurizer = DescriptorOnlyFeaturizer(
            aggregation=config.featurizer_params.get('aggregation', 'mean'),
            descriptor_set=config.featurizer_params.get('descriptor_set', 'basic'),
            use_counts=config.featurizer_params.get('use_counts', True),
        )
