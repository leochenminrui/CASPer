...
OUT=ROOT/'results/final_experiments';SEEDS=range(5);METS=['r2','rmse','mae','spearman']
...
def main():
 ...
 for sp in ['random','sequence_cluster']:
  for fs in list(NAME)[:7]:
   for seed in SEEDS:
    p=ROOT/f'results/final_experiments/raw_runs/primary_ablation/{sp}/seed_{seed}/{MID[fs]}/metrics.json';d=json.loads(p.read_text());t=d['test_metrics']
 ...
 for sp,pairs in {'random':[('A+B+C','A'),('A+B','A'),('A+B+C','A+B'),('A+C','A'),('B+C','B'),('B+C','C')],'sequence_cluster':[('A+B','A'),('A+B','A+B+C'),('A+B+C','A'),('A+C','A'),('B+C','B'),('B+C','C'),('A+B','ECFP'),('A+B','RDKit')]}.items():
  for x,y in pairs:
   for m in METS:
    def vals(fs):
     if fs in ['ECFP','RDKit']:
      return np.array([json.loads((ROOT/f'results/final_experiments/raw_runs/baselines/{sp}/seed_{s}/{MID[fs]}/metrics.json').read_text())['test_metrics'][m] for s in SEEDS])
