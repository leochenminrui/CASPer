#!/usr/bin/env python3
"""Generate all final revision figures exclusively from frozen result tables."""
...
RES=ROOT/'results/minor_revision_experiments'
OUT=ROOT/'revision_submission/manuscript/figures'
...
def shap_figs():
 ...
 t=pd.read_csv(RES/'shap/top_features.csv');t=t[(t.representation_id=='A+B+C')&(t.scope=='overall')].sort_values('rank').head(15);t.to_csv(DATA/'shap_top15.csv',index=False);fig,ax=plt.subplots(figsize=(7,5.5));z=t.iloc[::-1];ax.barh(z.feature,z.proportion,color=[{'A':'#0072B2','B1':'#E69F00','B2':'#009E73','B3':'#CC79A7','C':'#56B4E9'}[x] for x in z.subblock]);ax.set_xlabel('Mean absolute SHAP proportion across five seeds');ax.set_title('Top individual descriptors: complete representation');ax.grid(axis='x',ls='--',alpha=.25);save(fig,'figure_shap_top15_features')
...
def main():
 ablation();split_slope();estimators();shap_figs();time_forward();scaffold();predicted_observed();print(f'Figures written to {OUT}')
